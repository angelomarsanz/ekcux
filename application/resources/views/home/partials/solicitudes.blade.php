<div id="url-total" data-url-total={{ Request::fullUrl() }}></div> 
                        ...
                        <tbody>
                          @forelse($retiros as $transaction)
                            <tr>
                              ...
                              <td>{{$transaction->currency_symbol}} {{number_format($transaction->net, 2, ",", ".")}}</td>
                              <td>{{number_format($transaction->fee, 2, ",", ".")}}</td>
                    ...
                    <table class="table m-b-0">
                        ...
                        <tbody>
                          @forelse($fondeos_aceptados as $transaction)
                            <tr>
                              <td>
                                @if ($transaction->transaction_state_id == 6)
                                  <form action="{{route('post.confirmarTransferenciaFondeo', app()->getLocale())}}" method="post">                               
                                    {{csrf_field()}}
                                    <input type="hidden" name="tid" value="{{$transaction->id}}">
